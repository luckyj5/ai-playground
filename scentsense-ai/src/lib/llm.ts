...
function buyLinkFor(f: Fragrance): string {
  // Placeholder affiliate link structure – replace with a real affiliate id
  // (Amazon Associates, Sephora Rakuten, etc.) before going live.
  const q = encodeURIComponent(`${f.brand} ${f.name}`);
  return `https://www.amazon.com/s?k=${q}&tag=scentsense-20`;
}
